...
- **DevTools** — an in‑app panel to watch the fabric update live: named signals/computeds/effects, their values, who triggers whom, a temporal trigger‑trace, and a component/owner tree. `weave dev --devtools` puts it in the page; its **States** tab saves the screen you are on to a file, and `weave dev --state <name>` opens the app already in it.
- **Tooling that reads both files at once** — a template mistake is reported at its own line with the fix (`weave check --fix` applies the certain ones, and the editor offers the same on its lightbulb); the template can declare a missing name into `setup()`; renaming a binding renames the `const` behind it; `weave check --impact <file>` answers what actually renders a component; and `weave merge --install` teaches git to merge templates by their tree instead of by lines.
