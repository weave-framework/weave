...
check('list styles the selected row via [aria-selected] (accentSoft + accent marker)', /\.weave-list__row\[aria-selected=true\]\s*{[\s\S]*?background:\s*var\(--weave-list-selected-background\)[\s\S]*?border-inline-start-color:\s*var\(--weave-list-selected-marker\)/.test(cssStyles));
...
// The +/− marker is a lucide `plus`/`minus` <Icon> now, chosen in the template from the panel's
// open state — the CSS only hosts and sizes it. The FLIP is asserted in expansion.browser.ts,
// where the behaviour now lives.
check('expansion marker hosts a sized <Icon>', /\.weave-expansion__marker\s*{[^}]*--weave-icon-size:\s*var\(--weave-expansion-marker-width\)/.test(cssStyles) && !/\.weave-expansion__marker::after\s*{[^}]*content:\s*["']\+["']/.test(cssStyles));
...
check('table selected row = accentSoft tint + 2px accent inline-start border (via [aria-selected])', /\.weave-table__row\[aria-selected=true\]\s*>\s*\.weave-table__cell\s*{[\s\S]*?background:\s*var\(--weave-table-selected-background\)/.test(cssStyles) && /\.weave-table__row\[aria-selected=true\]\s*>\s*\.weave-table__cell:first-child\s*{[\s\S]*?border-inline-start-color:\s*var\(--weave-table-selected-marker\)/.test(cssStyles));
// The expand marker is a lucide chevron <Icon> now (same treatment as Tree), so the rotation
// targets `.weave-icon` rather than a `::before` drawing a ▸.
check('table sticky columns are position:sticky; expand chevron rotates on [aria-expanded]', /\.weave-table__cell--sticky-start,\s*\.weave-table__cell--sticky-end\s*{[\s\S]*?position:\s*sticky/.test(cssStyles) && /\.weave-table__expand-toggle\[aria-expanded=true\]\s+\.weave-icon\s*{[\s\S]*?transform:\s*rotate\(90deg\)/.test(cssStyles));
