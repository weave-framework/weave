...
Everything in `@weave/forms` is a **`Control`**. A `field`, a `group`, and a `fieldArray` all implement the same interface, which is why they nest to any depth (`form → fieldArray → group → field`) and aggregate validity/values/touched the whole way up. It's the Weave analog of Angular's `AbstractControl`.
...
~~~ts title="form.ts"
import { FORM_ERROR_KEY } from '@weave/forms'; // === '_form'
~~~
...
import { field, fieldArray, group, validators } from '@weave/forms';
