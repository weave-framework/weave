...
description: >-
  ...
  Angular/Nx workspaces, editor tooling (typescript-plugin / VS Code / WebStorm /
...
- **Language server** (`@weave-framework/language-server`, Volar) — template hover / go-to-definition / diagnostics on `.html`/`.weave`. Shipped by the VS Code extension and the WebStorm plugin.
...
`@weave-framework/nx` provides inferred targets, executors (`build`/`serve`/`check`), and generators (app/component). In a **mixed workspace** (Weave beside another framework, e.g. an Angular→Weave migration), a project keeps behaving like the other framework until its own config says otherwise. Three markers flip a project to Weave, plus a `project.json` target override:
