...
test('an attribute change updates the reactive prop', () => {
  ...
  const h: HTMLElement = host();
  const el: HTMLElement = document.createElement('w-greet-b');
...
test('a JS property set updates the reactive prop', () => {
  ...
  const h: HTMLElement = host();
  const el: HTMLElement & { label: string } = document.createElement('w-greet-c') as HTMLElement & { label: string };
  h.appendChild(el);
  el.label = 'Dot';
...
test('a kebab-case attribute maps to a camelCase prop', () => {
  const NameWidget: Component = defineComponent((ctx) => {
...
test('disconnecting unmounts (disposes) the component', () => {
  ...
  const h: HTMLElement = host();
  const el: HTMLElement = document.createElement('w-greet-d');
