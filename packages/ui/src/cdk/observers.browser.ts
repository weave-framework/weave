...
import { resizeSignal, mutationSignal } from '@weave-framework/ui/cdk';
...
test('mutationSignal: ticks on child + attribute changes', async () => {
  const el = box(10, 10);
  const changes = mutationSignal(el);
...
test('mutationSignal: stops after owner disposal', async () => {
  ...
  const changes = runInOwner(owner, () => mutationSignal(el));
